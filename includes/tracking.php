...
class PMPro_Matomo_Tracking {
    ...
    public function __construct() {
        ...
        if ((empty($this->tracker_url) || empty($this->site_id)) && class_exists('WP_Piwik')) {
            ...
            if (method_exists($wp_piwik, 'getOption')) {
                ...
                // Skip piwik_url unless we add our own tracking
                if (empty($this->tracker_url) && method_exists($wp_piwik, 'getMatomoUrl')) {
                    $this->tracker_url = rtrim($wp_piwik->getMatomoUrl() ?: '', '/');
                } elseif (empty($this->tracker_url) && method_exists($wp_piwik, 'getPiwikUrl')) {
                    $this->tracker_url = rtrim($wp_piwik->getPiwikUrl() ?: '', '/');
                }
            }
        }

        // Enable if we have what we need
    ...
    public function register_hooks() {
        add_action('wp_footer', [$this, 'add_tracking_code'], 20); // After WP-Piwik’s JS
    ...
    public function add_tracking_code() {
        ...
        // WP-Piwik handles base tracking; we just log for debug
        ?>
        <script type="text/javascript">
            console.log('PMPro Matomo: Page view tracking enabled via WP-Piwik - Site ID: <?php echo esc_js($this->site_id); ?>');
        </script>
        <?php
    }
    ...
    public function track_membership_signup($user_id, $order) {
        ...
        <script type="text/javascript">
            if (typeof _paq !== 'undefined') {
                ...
                console.log('PMPro Matomo: _paq not found - ensure WP-Piwik is active');
            }
        </script>
        <?php
    }
